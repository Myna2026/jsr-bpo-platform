...
function coAvatar(ag,size){ try{ if(_coCtx.avatarHtml) return _coCtx.avatarHtml(ag,size); if(typeof kbAvatarHtml==='function') return kbAvatarHtml(ag,size); }catch(e){} return '<div style="width:100%;height:100%;border-radius:50%;background:'+(ag&&ag.color||'#0A4A8F')+';color:#fff;display:flex;align-items:center;justify-content:center;font-weight:800">'+String((ag&&ag.name)||'C').charAt(0)+'</div>'; }
...
'.co{--acc:var(--brand,#0A4A8F);background:#fff;border:1px solid color-mix(in srgb, var(--acc,#0A4A8F) 22%, #e2e8f0);border-radius:20px;padding:22px;box-shadow:0 8px 26px rgba(15,23,42,.06);font-family:Inter,system-ui,sans-serif;color:#0f172a}',
'.co *{box-sizing:border-box} .co-hd{display:flex;gap:16px;align-items:center;margin-bottom:14px} .co-orb{flex:0 0 auto;width:60px;height:60px;border-radius:50%;overflow:hidden;background:#fff;border:3px solid color-mix(in srgb, var(--acc,#0A4A8F) 35%, #fff)} .co-orb svg,.co-orb img{width:100%;height:100%;display:block;object-fit:cover}',
...
/* Fokus-Modus */
'.co-focus{position:fixed;inset:0;z-index:100000;background:#F5F6F8;overflow:auto;font-family:Inter,system-ui,sans-serif;color:#0f172a;--acc:#0A4A8F} .co-focus *{box-sizing:border-box}',
...
'.co-prog{display:flex;gap:4px;padding:10px 18px 0;max-width:820px;margin:0 auto} .co-prog i{flex:1;height:6px;border-radius:3px;background:#e2e8f0} .co-prog i.ok{background:#059669} .co-prog i.half{background:#d97706} .co-prog i.no{background:#dc2626} .co-prog i.cur{background:var(--acc,#0A4A8F)}',
'.co-stage{max-width:820px;margin:0 auto;padding:22px 18px 60px}',
'.co-meta{font-size:12px;color:#64748b;font-weight:600;margin-bottom:8px} .co-meta b{color:var(--acc,#0A4A8F)} .co-meta .kind{display:inline-block;padding:1px 8px;border-radius:6px;background:#eef2f4;color:#334155;margin-left:6px}',
...
'.co-opt:hover{border-color:color-mix(in srgb, var(--acc,#0A4A8F) 50%, #e2e8f0)} .co-opt.on{border-color:var(--acc,#0A4A8F);background:color-mix(in srgb, var(--acc,#0A4A8F) 8%, #fff)} .co-opt.right{border-color:#059669;background:#ecfdf5} .co-opt.wrong{border-color:#dc2626;background:#fef2f2}',
'.co-opt .k{flex:0 0 auto;width:30px;height:30px;border-radius:50%;background:#f1f5f9;color:#334155;font-weight:800;text-align:center;line-height:30px;font-size:13px} .co-opt.on .k{background:var(--acc,#0A4A8F);color:#fff} .co-opt.right .k{background:#059669;color:#fff} .co-opt.wrong .k{background:#dc2626;color:#fff} .co-opt .why{display:block;font-size:12.5px;color:#64748b;margin-top:3px}',
'.co-in{width:100%;padding:14px 16px;border:2px solid #e2e8f0;border-radius:14px;font-size:17px;font-family:inherit;min-height:52px;background:#fff} .co-in:focus{outline:none;border-color:var(--acc,#0A4A8F)} .co-ta{min-height:130px;resize:vertical;line-height:1.5}',
...
'.co-fb .h{font-size:16px;font-weight:800;margin-bottom:6px} .co-fb .x{font-size:15px;color:#0f172a;line-height:1.55;white-space:pre-wrap} .co-fb .model{margin-top:10px;padding:12px 14px;border-radius:12px;background:#fff;border:1px dashed color-mix(in srgb, var(--acc,#0A4A8F) 45%, #cbd5e1)} .co-fb .model .l{font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.05em;color:var(--acc,#0A4A8F);margin-bottom:4px} .co-fb .model .t{font-size:15px;font-style:italic;line-height:1.5}',
...
'.co-res{text-align:center;padding:10px 0} .co-res .big{font-size:56px;font-weight:800;color:var(--acc,#0A4A8F);line-height:1;font-variant-numeric:tabular-nums} .co-res .sub{font-size:15px;color:#334155;margin-top:8px}',
...
function renderCoach(){
  ...
  Promise.all([availP, sb.from('partner_agents').select('*').eq('project_id',pid).maybeSingle()]).then(function(r){
    _coAvail=(r[0]&&r[0].data)||{}; _coAg=(r[1]&&r[1].data)||(_coCtx.fallbackAgent?_coCtx.fallbackAgent(pid):(typeof kbFallbackAgent==='function'?kbFallbackAgent(pid):{name:'Coach',color:'#0A4A8F'}));
...
function coStartScreen(host){
  ...
  host.innerHTML='<div class="co" style="--acc:'+(ag.color||'#0A4A8F')+'">'
...
function coOpenFocus(){ var old=document.getElementById('coFocus'); if(old) old.remove();
  var f=document.createElement('div'); f.id='coFocus'; f.className='co-focus'; f.style.setProperty('--acc',(_coAg.color||'#0A4A8F'));
...
function coTick(){ var el=document.getElementById('coClock'); if(!el) return; var show=!(_coAvail&&_coAvail.settings&&_coAvail.settings.show_timer===false); if(!show){ el.innerHTML=''; return; }
  ...
  el.className='co-clock'+(left<0?' over':''); el.innerHTML='<svg width="36" height="36" viewBox="0 0 36 36"><circle cx="18" cy="18" r="'+r+'" fill="none" stroke="#e2e8f0" stroke-width="4"/><circle cx="18" cy="18" r="'+r+'" fill="none" stroke="'+(left<0?'#dc2626':'var(--acc,#0A4A8F)')+'" stroke-width="4" stroke-linecap="round" stroke-dasharray="'+c+'" stroke-dashoffset="'+(c*frac)+'" transform="rotate(-90 18 18)"/></svg><span class="t">'+txt+'</span>';
...
function coConfirm(title,text,yes,no,onYes){ var o=document.createElement('div'); o.className='co-over'; o.style.setProperty('--acc',(_coAg&&_coAg.color)||'#0A4A8F'); o.innerHTML='<div class="box"><h3>'+coEsc(title)+'</h3><p>'+coEsc(text)+'</p><div class="co-nav" style="justify-content:center;margin-top:0"><button class="co-btn sec" id="coNo">'+coEsc(no)+'</button><button class="co-btn" id="coYes">'+coEsc(yes)+'</button></div></div>'; document.body.appendChild(o);
