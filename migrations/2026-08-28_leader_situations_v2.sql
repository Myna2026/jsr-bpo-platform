-- v2: reichert die schwachen Agenten an, damit der Anstoß EINORDNET und einen HEBEL nennt (was/bedeutet/tun):
--   value + prev (Trend zur Vorwoche) + team_avg (Vergleich) + tenure_weeks + last_fb_days
--   + acw_sec/aht_sec/hold_sec (Ursache: lange Nachbearbeitung/Halten zieht die AHT hoch).
-- Alles belegbar aus den Wochendaten. Rest wie v1. Service-role only.
...
begin
  ...
  weak as (select v.project_id, v.skill, jsonb_agg(jsonb_build_object(
      'name',v.name,'kpi',v.kpi,'value',round(v.value,2),'prev',v.prev,'team_avg',v.team_avg,
