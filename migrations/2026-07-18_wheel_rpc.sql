-- =============================================================================
-- Gluecksrad: server-autoritativer Dreh via RPC   2026-07-18
-- =============================================================================
-- Etappe G2b: SECURITY-DEFINER-Funktion spin_wheel(). Bildet die G2-JS-Logik
-- (hr.html calculateSpinResult) in SQL ab, damit der MA drehen kann, OHNE selbst
-- in wheel_spins schreiben zu duerfen (RLS = HR write). Die Funktion laeuft als
-- Owner -> umgeht RLS fuer den Insert. Der aufrufende MA wird serverseitig ueber
-- app_users.employee_id (auth.uid()) ermittelt -> KEIN emp_id-Parameter, damit
-- niemand fuer andere drehen kann.
--
-- Ablauf (wie G2): Tages-Check (1 Dreh/Tag, unique emp_id+spin_date) -> Special
-- (start_date<=heute<=end_date) sonst Monatsbudget -> Restbudget -> Fairness-Gewicht
-- (Wartestand +0.25/Woche, KPI-Faktor gut x1.15/schlecht x0.85, Garantie ab 4 Wochen
-- +2.0) -> gewichtete Gewinnhoehe (klein haeufig) -> Budget-Bremse -> Insert (auch 0).
-- Rueckgabe jsonb {amount, source, message}.
--
-- Idempotent (create or replace). Anzuwenden im Supabase SQL Editor (hier NICHT ausgefuehrt).
-- Voraussetzung: 2026-07-18_wheel_budget.sql (G1) ist ausgefuehrt.
-- =============================================================================
...
-- Nur eingeloggte Nutzer duerfen drehen (der MA wird intern via auth.uid() aufgeloest).
...
-- =============================================================================
-- (a) Funktion existiert + ist SECURITY DEFINER:
--     select proname, prosecdef from pg_proc where proname='spin_wheel';   -- prosecdef=t
-- (b) Als eingeloggter MA testen (im SQL Editor mit gesetztem auth.uid() / im Client):
--     select public.spin_wheel();   -- -> {"amount":..,"source":"..","message":".."}
-- (c) Zweiter Aufruf am selben Tag -> alreadySpun:true.
-- =============================================================================
