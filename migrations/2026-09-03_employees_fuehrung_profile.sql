-- Punkt 2/3/4: Fuehrungs-Profil + Peer-Gehaltsregel + gelockerte Status-Schranke in employees_masked.
--
-- Punkt 4: Die Zeilen-Schranke 'nur active/training fuer projektgebundene Seher' entfaellt,
--   damit Projektleiter ihr Projekt VOLLSTAENDIG sehen (auch frisch Vertragte / Gekuendigte).
-- Punkt 2: Neues Spaltenprofil 'fuehrung' = volle Zeile MINUS bank MINUS id_number
--   (Gehalt/Vertrag/Notizen/Verwarnungen ja, Bank+Ausweis nein).
-- Punkt 3: Peer-Gehaltsregel im fuehrung-Zweig: Gehalt nur sichtbar, wenn salary='all' UND
--   der Ziel-Rang UNTER dem Betrachter-Rang liegt (ai_position_rank(Ziel) < perm_caller_rank).
--   Damit sieht ein Projektleiter (Rang 40) das Gehalt seiner Agenten (10)/Teamleiter (30),
--   aber NICHT das des gleichrangigen Projektleiters (40). Management/GF sind schon ueber die
--   Projekt-Achse (kein Projekt) draussen; is_protected_employee bleibt als Schutzriegel.
--
-- Definition uebernommen aus pg_get_viewdef(employees_masked) mit exakt diesen drei Eingriffen.
...
CREATE OR REPLACE VIEW public.employees_masked AS
 SELECT r.id,
   ...
   FROM ( SELECT e_1.id,
           ...
           FROM employees e_1
          WHERE perm_mode(auth.uid(), 'emp'::text) <> 'none'::text AND perm_proj_ok(auth.uid(), 'emp'::text, e_1.project_id, NULL::text)) e
     CROSS JOIN LATERAL ( SELECT
                CASE
                    WHEN is_management() OR is_finance() THEN to_jsonb(e.*)
                    WHEN (perm(auth.uid(), 'emp'::text) ->> 'columns'::text) = 'voll'::text THEN
                    CASE
                        ...
                        WHEN (perm(auth.uid(), 'emp'::text) ->> 'salary'::text) <> 'all'::text THEN to_jsonb(e.*) - 'fixed_salary'::text - 'hourly_rate'::text - 'salary_currency'::text
                        ELSE to_jsonb(e.*)
                    END
                    WHEN (perm(auth.uid(), 'emp'::text) ->> 'columns'::text) = 'fuehrung'::text THEN
                    CASE
                        WHEN is_protected_employee(e.id) THEN to_jsonb(e.*) - 'bank'::text - 'id_number'::text - 'fixed_salary'::text - 'hourly_rate'::text - 'salary_currency'::text - 'contract'::text
                        WHEN (perm(auth.uid(), 'emp'::text) ->> 'salary'::text) = 'all'::text AND ai_position_rank(e."position") < perm_caller_rank(auth.uid()) THEN to_jsonb(e.*) - 'bank'::text - 'id_number'::text
                        ELSE to_jsonb(e.*) - 'bank'::text - 'id_number'::text - 'fixed_salary'::text - 'hourly_rate'::text - 'salary_currency'::text
                    END
                    WHEN (perm(auth.uid(), 'emp'::text) ->> 'columns'::text) = 'personal'::text THEN to_jsonb(e.*) - 'fixed_salary'::text - 'hourly_rate'::text - 'salary_currency'::text
