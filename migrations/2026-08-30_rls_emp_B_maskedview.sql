-- Rechte Schnitt 6, Scheibe 5 (Mitarbeiter), TEIL B: Zeilenfilter von employees_masked aufs Modell.
-- NUR der innere WHERE-Filter wird ersetzt; die drei Spaltenstufen (mgmt/finance voll · hr voll außer
-- Management-MA-Gehalt/Bank/Vertrag/Ausweis · Leads operativer Ausschnitt) bleiben BIT-GENAU erhalten
-- (fehlende Spaltenstufen-Achse ist Taxonomie-Merkposten). View-Optionen (security_invoker=false,
-- security_barrier=true) erhalten → läuft weiter als Owner, umgeht Basis-RLS zum Maskieren.
-- Zeilen-Parität verifiziert IST-exakt: mgmt/hr=69(alle), Leads=24(eigenes Projekt active/training), MA=0.
...
create or replace view public.employees_masked with (security_invoker=false, security_barrier=true) as
 SELECT r.id,
   ...
   FROM ( SELECT e_1.id,
           ...
           FROM employees e_1
          WHERE public.perm_mode(auth.uid(), 'emp'::text) <> 'none'::text AND public.perm_proj_ok(auth.uid(), 'emp'::text, e_1.project_id, NULL::text) AND (public.perm_allowed_projects(auth.uid(), 'emp'::text) IS NULL OR e_1.status = ANY (ARRAY['active'::text, 'training'::text]))) e
...
-- ── ROLLBACK: employees_masked neu erstellen mit der alten WHERE-Bedingung ──
--   is_management() OR is_hr() OR is_finance() OR is_planner() AND e_1.project_id = get_my_employee_project_id()
--   AND (e_1.status = ANY (ARRAY['active'::text, 'training'::text]))
-- (voller alter View-Text: git show vor diesem Commit / pg_get_viewdef-Dump; Spaltenstufen sind unverändert).
