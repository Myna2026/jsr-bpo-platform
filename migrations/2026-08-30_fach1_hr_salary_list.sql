-- Fachentscheidung 1 (User 2026-08-30): hr sieht Gehälter in der Mitarbeiter-Liste. hr sah schon nicht-geschützte
-- Gehälter (Masked-View Branch B = volle Zeile), der Seed 'keins' steuerte nur den ungenutzten Basis-Direktzugriff.
-- Ehrlich + robust: (1) GF-Freigabe hängt an is_management/is_finance statt an der Gehalt-Achse (sonst gäbe hr's
-- neues salary=all versehentlich die GF-Gehälter frei), (2) die Gehalt-Achse wird für die voll-Stufe wirksam
-- (voll ohne salary=all → Gehalt raus), (3) hr emp-Gehalt-Seed none→all. GF-Schutz (is_protected_employee) bleibt.
...
update public.role_permissions set salary='all' where area_key='emp' and role_key='hr';

-- ROLLBACK: update role_permissions set salary='none' where area_key='emp' and role_key='hr';
-- + employees_masked mit altem Branch A (columns='voll' AND salary='all') neu erstellen (git).
