-- Situationsanalyse je (Projekt, Skill)-Gruppe für die Teamleiter-Anstöße (Schnitt 5).
-- Team = Skill-Gruppe (kein 1:1-Team je Leiter). Leiter = Position 'Teamleiter' auf der Gruppe.
-- Belegbare Fakten (NIE geraten): weak/strong = KPI-Wert der letzten Woche im Band 'Kritisch'/'Schlecht'
-- bzw. 'Sehr gut' (Klassifikation gegen kpi_config.thresholds, absolut, nicht relativ). Dazu Teamgröße,
-- Abwesenheiten dieser Woche, Neuzugänge (<28 Tage), Feedback-Lücken (>=28 Tage ohne Gespräch).
-- Der Text/Katalog entscheidet später, was daraus wird; die RPC liefert nur die Wahrheit. Service-role only.
...
begin
  ...
  vals as (
    select t.project_id, t.skill, t.name, kc.name as kpi, ke.value,
  ...
  weak as (select v.project_id, v.skill, jsonb_agg(jsonb_build_object('name',v.name,'kpi',v.kpi,'value',round(v.value,2))) as arr
    from vals v where v.band in ('Kritisch','Schlecht') group by v.project_id, v.skill),
  strong as (select v.project_id, v.skill, jsonb_agg(jsonb_build_object('name',v.name,'kpi',v.kpi,'value',round(v.value,2))) as arr
