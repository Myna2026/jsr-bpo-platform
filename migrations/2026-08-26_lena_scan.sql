-- Lenas Datenpflege-Wächter: findet live Auffälligkeiten rund um Mitarbeiter. MELDET nur (keine Sanktion).
-- is_admin() (Management+HR); Service-Role (auth.uid()=NULL, z. B. usage-digest) darf mitzählen. Anon gesperrt.
-- Kategorien: vertrag_ohne_daten, ausweis_fehlt, bank_fehlt, urlaubsantrag_liegt, abwesenheit_unplausibel, portalzugang_fehlt.
...
begin
  ...
  return query
  -- Verträge ohne Daten (Vertragsbeginn fehlt)
  ...
  union all
  -- Bankdaten fehlen (nur wer in Bezahlung ist)
  ...
  union all
  -- Liegengebliebene Urlaubsanträge (offen, älter als 7 Tage)
  ...
  union all
  -- Unplausible Abwesenheiten (Ende vor Beginn)
  ...
  union all
  -- Fehlende Portalzugänge (aktiv/Schulung ohne verknüpften aktiven app_user)
...
grant execute on function public.lena_scan() to authenticated;
