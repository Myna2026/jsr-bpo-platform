...
Deno.serve(async (req)=>{
  ...
  const bh = berlinHour();
  // Zentrale Zeitsteuerung: feuert nur zur geplanten Zeit (Standard 07:00, Mo–Fr).
  const _sched = await getSchedule(sb, "clara_digest");
  if(!force && sp.get("dry")!=="1" && !scheduleDue(_sched)) return json({skipped:"not-scheduled", berlinHour:bh});
